/**
 * CommentsTableMobile Component
 * Mobile card view for displaying comments on small screens
 *
 * @author Thang Truong
 * @date 2025-11-27
 */

interface CommentUser {
  id: string
  firstName: string
  lastName: string
  email: string
  role: string
}

interface Comment {
  id: string
  uuid: string
  content: string
  projectId: string | null
  user: CommentUser
  likesCount: number
  isLiked: boolean
  createdAt: string
  updatedAt: string
}

interface CommentsTableMobileProps {
  comments: Comment[]
  onEdit: (commentId: string) => void
  onDelete: (commentId: string) => void
}

/**
 * CommentsTableMobile Component
 * Renders comments as cards for mobile devices
 *
 * @author Thang Truong
 * @date 2025-11-27
 * @param comments - Array of comment objects to display
 * @param onEdit - Callback function when edit button is clicked
 * @param onDelete - Callback function when delete button is clicked
 * @returns JSX element containing mobile card view
 */
const CommentsTableMobile = ({ comments, onEdit, onDelete }: CommentsTableMobileProps) => {
  /**
   * Format date to Melbourne timezone
   *
   * @author Thang Truong
   * @date 2025-11-27
   * @param dateString - ISO date string from database
   * @returns Formatted date string in Melbourne timezone
   */
  const formatDateToMelbourne = (dateString: string | null | undefined) => {
    try {
      if (!dateString) return 'N/A'
      const date = new Date(dateString)
      if (isNaN(date.getTime())) return 'Invalid Date'
      return date.toLocaleDateString('en-AU', {
        timeZone: 'Australia/Melbourne',
        year: 'numeric',
        month: 'short',
        day: 'numeric',
      })
    } catch {
      return 'Invalid Date'
    }
  }

  return (
    /* Mobile Cards Container */
    <div className="space-y-3 p-3">
      {comments.map((comment) => (
        <div key={comment.id} className="bg-gray-50 rounded-lg p-4 border border-gray-200">
          <div className="flex items-start justify-between mb-3">
            <div className="flex-1 min-w-0">
              <h3 className="text-sm font-semibold text-gray-900 line-clamp-2">{comment.content}</h3>
              <p className="text-xs text-gray-600 mt-1">
                By: {comment.user.firstName} {comment.user.lastName}
              </p>
            </div>
          </div>
          <div className="text-xs text-gray-500 mb-3 space-y-1">
            <p>ID: {comment.id}</p>
            {comment.projectId && <p>Project ID: {comment.projectId}</p>}
            <p>Likes: {comment.likesCount}</p>
            <p>Created: {formatDateToMelbourne(comment.createdAt)}</p>
            <p>Updated: {formatDateToMelbourne(comment.updatedAt)}</p>
          </div>
          <div className="flex flex-col sm:flex-row gap-2 mt-3">
            <button
              onClick={() => onEdit(comment.id)}
              className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 bg-blue-200 text-blue-700 rounded-md hover:bg-blue-100 transition-colors text-xs font-medium touch-manipulation"
              aria-label="Edit comment"
            >
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
              </svg>
              <span>Edit</span>
            </button>
            <button
              onClick={() => onDelete(comment.id)}
              className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 bg-red-200 text-red-700 rounded-md hover:bg-red-100 transition-colors text-xs font-medium touch-manipulation"
              aria-label="Delete comment"
            >
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
              </svg>
              <span>Delete</span>
            </button>
          </div>
        </div>
      ))}
    </div>
  )
}

export default CommentsTableMobile

