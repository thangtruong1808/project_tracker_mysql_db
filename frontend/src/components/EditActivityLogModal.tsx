/**
 * EditActivityLogModal Component
 * Modal dialog for editing existing activity log entries
 *
 * @author Thang Truong
 * @date 2024-12-24
 */
...
import { UPDATE_ACTIVITY_MUTATION } from '../graphql/mutations'
import { ACTIVITIES_QUERY, USERS_QUERY, PROJECTS_QUERY, TASKS_QUERY } from '../graphql/queries'
import EditActivityLogFormFields from './EditActivityLogFormFields'
...
const EditActivityLogModal = ({ activity, isOpen, onClose, onSuccess }: EditActivityLogModalProps) => {
  ...
  const {
    ...
    reset,
  } = useForm<EditActivityLogFormData>({
    mode: 'onBlur',
    defaultValues: {
      userId: '',
      targetUserId: '',
  ...
  const onSubmit = async (data: EditActivityLogFormData) => {
    if (!activity) return
    setError('')
    try {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
      <div className="bg-white rounded-lg shadow-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h2 className="text-xl font-semibold text-gray-900">Edit Activity Log</h2>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6">
          <EditActivityLogFormFields register={register} errors={errors} users={users} projects={projects} tasks={tasks} />
...
export default EditActivityLogModal
