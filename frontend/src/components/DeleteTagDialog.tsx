...
/**
 * DeleteTagDialog Component
 * Renders a confirmation dialog for deleting a tag
 *
 * @param tag - Tag object to delete (null when closed)
 * @param isOpen - Whether the dialog is open
 * @param onClose - Callback when dialog is closed
 * @param onSuccess - Callback when tag is successfully deleted
 * @returns JSX element containing delete confirmation dialog
 */
const DeleteTagDialog = ({ tag, isOpen, onClose, onSuccess }: DeleteTagDialogProps) => {
  const { showToast } = useToast()
  const [isDeleting, setIsDeleting] = useState(false)

  /**
   * Delete tag mutation
   * Refetches tags list after successful deletion
   */
   ...
   * Deletes the tag and shows success/error message
   */
  const handleDelete = async () => {
    ...
    try {
      ...
      await showToast('Tag deleted successfully', 'success', 7000)
      onSuccess()
      onClose()
    } catch (error: unknown) {
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
        ...
        <div className="p-6">
          <p className="text-gray-700 mb-4">
            ...
            <span className="font-semibold">{tag.name}</span>? This action cannot be undone.
          </p>
          <div className="bg-gray-50 rounded-lg p-4 mb-4">
            ...
            {tag.type && (
              <p className="text-sm text-gray-600 mb-1">
                <span className="font-medium">Type:</span> {tag.type}
              </p>
            )}
            {tag.category && (
              <p className="text-sm text-gray-600">
                <span className="font-medium">Category:</span> {tag.category}
              </p>
            )}
          </div>
        </div>
